...
median_salaries = all_players_adjusted %>%
  ...
  group_by(gender, player_positions) %>%
  summarize(
    med_salary = median(wage_eur, na.rm = TRUE)
  )
...
### Ranking of wages depending on the country where the player is located in

```{r}
 ggplot(all_players, aes(x = fct_relevel(performance_rating, "High", after = 2), y = percen
  geom_boxplot(color = "black", fill = "lightBlue") +
  ggtitle("Boxplots of % increases according to performance ratings and missing values") +
4
t_incr)) +
 xlab("Performance Rating") +
ylab("Percetange Increase (%)") +
coord_flip()
```