...
We are first going to look at the overall distribution of players' heights for both male and female. We expect the median height of the men to be larger than the median height of women. For this analysis, we will only use the most recent year of data we have, specifically 2022. 
...
The Cleveland Dot Plot above provides many interesting insights. First, we can confirm the above conclusion that the male players are taller than the female players. Furthermore, we can see that this is true for every single position. 
...
We again can see that goalkeepers and defensive players tend to be the tallest. The distribution for each position for the male players appears to be normally distributed. Interestingly, there seems to be some bimodality and uniform distribution of the heights for some of the female player positions. For example, there are two groups of players for the RB position, one who are taller and ones who are shorter. For positions like CAM, RW, ST, and CM, the distribution seems almost uniform, with heights ranging over a 15 or 20 cm range. It looks like the heights of players might not be as crucial of an attribute for the women's game as speed, for example. The women's game is often played more on the ground than in the air, so speed and quickness could be more advantageous than height regardless of the position (except goalkeeper and some defensive positions). 
...
We can see from this graph that the wages grow with the overall rating of the player, that sounds logical. To the contrary, the age does not look like having a strong influence on the wages.
Now, let's perform a linear regression to compare the influence of each feature.

### Explaining wages through Linear Regression

We are trying to explain the wages given the data available in the other columns through a linear regression. When completed, we keep only the 30 most significant coefficients (with the lowest p-value) and we plot them in a Cleveland dot plot to compare them.
...
We can see that the club name is clearly the best indicator to estimate the salary of a player. Indeed, clubs like PSG or Manchester City are well known to be extremely rich, because they benefit from the support of a country (Qatar and Saudi Arabia) as investors. Therefore, they are able to pay their players a lot, and this seems to have a much higher influence than the actual level of the player.
...
Once this observation is done, let's remove the feature "league_name" to observe the influence of the other features.
...
Here, we can notice that the position of the player becomes to most important feature. For instance, Center Forwards (CF) are much better paid than Right Defensive Midfielders (RDM), probably because the position is more "spectacular" and marketable (they will sell more jerseys). We can also see that the international reputation is a very important feature to explain the salary. It can be justified by the fact that football is an open competitive market: if a player with a good reputation is not well paid, he is likely to get offers from other clubs and change during the summer.
...
### Explaining values through a scatter plot

First, let's have an insight by plotting the values in function of the "overall" rating and their age.
...
We can see from this graph that the values grow with the overall rating of the player, that sounds logical (the more gifted a player, the higher his value). But unlike the wages study, here, the age seems to have a strong influence on the values! Indeed, for the same overall rating, younger players clearly have a higher value.

Now, let's perform a linear regression to compare the influence of each feature.

### Explaining values through Linear Regression
...
Like for the last study, the club name is the best way to estimate the market value of a player. Indeed, it is known that the most prestigious clubs are able to attract the most valuable players.
Once this observation is done, let's remove the feature "club_name" to observe the influence of the others.
...
We can see that Brazil, Argentina, Spain, France and Germany are the most represented countries, with around 1000 players each. But even though it is not the national sport, there are about 400 Americans and Chinese present in the game! To the contrary, football is very popular in Africa, but very few African players are represented. It is probably because the African leagues are not yet in the game.

### Top 1000 players

It may be interesting to see if the countries with the most players are also the countries with the best players. For this purpose, we plot the same chart, but only with the 1000 players with the highest overall ranking. 
