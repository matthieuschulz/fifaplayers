...
fig <- plot_ly(
  male_22_field, x = ~overall, y = ~wage_eur,
  text = ~paste("Name: ", short_name),
  color = ~age
...
ggplot(data=results, aes(x = Estimate, y = fct_reorder(Skill, Estimate)))+
  ...
  theme_linedraw()
```
Finally, we are left with only the pure skills and personality of the player. We said that the international reputation was important, but we can now observe that ratings like "overall" or "skill_moves" are also good indicators. The fact that "Preferred_foot_right" is important seems to show that right footed players are more paid than left footed! Finally, your age seems to slightly negatively impact your wages. 
